...
setwd('/Users/hollykular/documents/FYP/code/HK/Behavioral/WM_DistractV3/Data/V11')
data1 <-read_csv('WM_pilot_220512.csv')
...
```{r}
# check for non-response numbers exclude subjects with greater than 10% (trials NA response>10)
nonrespcheck <- data1 %>% group_by(subject) %>% count(resp)
nonrespcheck <- nonrespcheck%>% filter(is.nan(resp))
# subject 1 and 2 had no non response
for(i in 1:nrow(nonrespcheck)){if (nonrespcheck$n[i] >19) {print(paste0('exclude subject ', nonrespcheck$subject[i], ' no response'))}} # prints out warning to exlude subject who didn't respond to more than 5% of trials
...
rm(FAavg)
# subj 5 and 7 are the outliers here
```
...
dplot1 <- d1 %>%
  ...
  scale_x_continuous(breaks = seq(1,s,1))+
  annotate(geom='text', x = c(1), y=2.5, label= 'inf', color = 'white')+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5), legend.position = 'none')
dplot1
...
#
### Raw performance
#### i.e. response must be exact to get correct, orientation discrimination
#### get hits
```{r}
# count hits
...
names(missing) <- c('subject', 'contrast','discrim', 'n')
hit3<- rbind(hit3, missing) # add back missing data
hit3 <- hit3[order(hit3$subject, hit3$contrast, hit3$discrim),] # reorder
rm(missing)
...
missing <- data.frame(13, 0.8, 0, 0)
names(missing) <- c('subject', 'contrast','discrim', 'n')
FA3<- rbind(FA3, missing) # add back missing data
missing <- data.frame(8, 0.8, 0, 0)
names(missing) <- c('subject', 'contrast','discrim', 'n')
FA3<- rbind(FA3, missing) # add back missing data
FA3 <- FA3[order(FA3$subject, FA3$contrast, FA3$discrim),] # reorder
rm(missing)
...
hit5<- rbind(hit5, missing) # add back missing data
rm(missing)
hit5 <- hit5[order(hit5$subject, hit5$setsize, hit5$discrim),] # reorder
# calculate rate
...
names(missing) <- c('subject', 'setsize','discrim', 'n')
FA5<- rbind(FA5, missing) # add back missing data
FA5 <- FA5[order(FA5$subject, FA5$setsize, FA5$discrim),] # reorder
rm(missing)
